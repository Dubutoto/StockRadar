...
public class Rtx4060TiAllCacheServiceV3CompletableFuture {
    ...
    public List<ProductResponseDto> getRtx4060TiInfo() {
        try {
            boolean redisAvailable = redisSmtpService.checkRedisConnection();
            ...
            // Redis가 사용 가능한 경우에만 캐시 관련 로직 실행
            if (redisAvailable) {
                // 2. 제품 ID 목록
                ...
                // Redis가 사용 불가능한 경우 DB 결과를 그대로 반환
                log.info("Redis 사용 불가능: DB 결과를 직접 반환합니다.");
                return products;
            }
