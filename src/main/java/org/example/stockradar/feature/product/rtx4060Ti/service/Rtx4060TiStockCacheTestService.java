...
public class Rtx4060TiStockCacheTestService {
    ...
    private AtomicInteger totalRedisOperations = new AtomicInteger(0);
    private AtomicInteger totalDbOperations = new AtomicInteger(0);
    ...
    public Integer getCachedStockStatus(Long productId) {
        totalRedisOperations.incrementAndGet(); // Redis 접근 카운트 증가
        String cacheKey = createStockCacheKey(productId);
        Object cachedValue = redisTemplate.opsForValue().get(cacheKey);

        if (cachedValue != null) {
            cacheHits.incrementAndGet(); // 캐시 히트 카운트 증가
            log.info("Cache HIT - Product ID: {}", productId);
            return (Integer) cachedValue;
        }

        cacheMisses.incrementAndGet(); // 캐시 미스 카운트 증가
        totalDbOperations.incrementAndGet(); // DB 접근 카운트 증가
        log.info("Cache MISS - Product ID: {}, DB query executed", productId);
    ...
    public Map<String, Object> getCacheStats() {
        ...
        stats.put("hitRatio", total > 0 ? (double)hits / total * 100 : 0);
        stats.put("redisOperations", totalRedisOperations.get());
        stats.put("dbOperations", totalDbOperations.get());
