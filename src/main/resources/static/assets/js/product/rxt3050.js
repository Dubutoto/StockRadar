document.addEventListener('DOMContentLoaded', function () {
    // RTX 3050 제품 정보를 가져오는 함수
    function fetchRtx3050Products() {
        fetch('/product/api/gpu/rtx3050')
            .then(response => {
                // 응답 상태 코드 확인
                if (!response.ok) {
                    // 에러 응답을 JSON으로 파싱
...
// 제품 데이터를 테이블에 표시하는 함수
    function displayProducts(products) {
        ...
        products.forEach(product => {
            ...
            statusCell.textContent = product.availability > 0 ? 'In Stock' : 'Out of Stock';
