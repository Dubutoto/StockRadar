...
def fetch_with_retries(session, url, max_retries=3, timeout=15):
    ...
    for attempt in range(max_retries):
        try:
            time.sleep(random.randint(1, 3))  # 1~3초 사이 랜덤 딜레이
...
def get_amazon_data(session, url):
    """
    Amazon 크롤링 (Requests + BeautifulSoup)
    - 상품명: #productTitle
    - 재고: #availability 안의 텍스트에 "in stock", "only", "order soon" 등이 있으면 True
...
def get_ssg_data(session, url):
    """
    SSG 크롤링 (Requests + BeautifulSoup)
    - 상품명: <span.cdtl_info_tit_txt> 혹은 <h2.cdtl_info_tit>
    - 재고: "일시품절" 문구가 있으면 False
    - 가격: <em.ssg_price> 또는 <span.ssg_price>
    """
...
def get_11st_data(session, url):
    """
    11번가 크롤링 (Requests + BeautifulSoup)
    - 상품명: <h1 class="title">...</h1>
    - 재고: <span class="text_em_sm">에 "품절"이 있으면 False
